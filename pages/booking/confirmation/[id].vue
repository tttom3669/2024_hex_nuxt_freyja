...
const { data } = await useFetch(`/api/v1/orders/${bookingId}`, {
  method: 'GET',
...
const daysCount = () => {
  const differenceTime =
    new Date(orderData.value.checkOutDate).getTime() -
    new Date(orderData.value.checkInDate).getTime();
        ...
        <div class="col-12 col-md-7">
          ...
          >
            ...
            <div class="text-neutral-0 fs-1">
              <h1 class="fw-bold">恭喜，{{ orderData.userInfo.name }}！</h1>
          ...
          <div class="d-flex flex-column align-items-md-start">
            ...
            <button
              @click="navigateTo(`/user/${userResult.result._id}`)"
        ...
        <div class="col-12 col-md-5">
          ...
          >
            ...
            <section class="d-flex flex-column gap-6">
              ...
              >
                <p class="mb-0">
                  {{ orderData.roomId.name }}，{{ daysCount() }} 晚
                </p>
                <span
                  class="d-inline-block mx-4 bg-neutral-80"
                  style="width: 1px; height: 18px"
                />
                <p class="mb-0">住宿人數：{{ orderData.peopleNum }} 位</p>
              </h3>

              <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
                <p class="title-deco mb-2">
                  入住：{{ formatDate(orderData.checkInDate) }}，15:00 可入住
                </p>
                <p class="title-deco mb-0">
                  退房：{{ formatDate(orderData.checkOutDate) }}，12:00 前退房
                </p>
              </div>

              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                NT$ {{ orderData.roomId.price.toLocaleString('zh-TW') }}
              </p>
            </section>
            ...
            <section>
              ...
              >
                <li
                  v-for="(facility, idx) in orderData.roomId.facilityInfo"
                  :key="idx"
            ...
            <section>
              ...
              >
                <li
                  v-for="(amenity, idx) in orderData.roomId.amenityInfo"
                  :key="idx"
                  class="flex-item d-flex gap-2"
                >
                  <Icon
